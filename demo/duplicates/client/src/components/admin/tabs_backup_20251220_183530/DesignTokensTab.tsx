...
export default function DesignTokensTab() {
    ...
    const { data: remoteTokens, isLoading, refetch } = useQuery({
...
// Simple Badge component since we didn't import one
function Badge({ variant, className, children }: any) {
    ...
    const vars = variant === 'outline' ? "border text-foreground" : "bg-primary text-primary-foreground hover:bg-primary/80";
    return <div className={`${base} ${vars} ${className}`}>{children}</div>;
}
